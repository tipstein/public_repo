...
{
    DateOnly date = new DateOnly(DOB.Year, DOB.Month, DOB.Day);
    
    var queryDid =
        from p in _db.Departments
        where p.Subject == departmentAbbrev
        select p.DId;

    var qUid = from u in _db.Users
        select u.SId;

    int sid = 1;
    
    while(qUid.Contains(sid))
    {
        sid += 1;
    }
    ...
    int dAbrev = queryDid.FirstOrDefault();
    ...
    switch (role)
    {
        case "Professor":
        {
            ...
            User new_use = new User();

            new_use.UId = uId;
            new_use.Name = firstName;
            new_use.Surname = lastName;
            new_use.Dob = date;
            new_use.DId = dAbrev;
            new_use.UserType = uId.Substring(0,1);
            new_use.SId = sid;
            _db.Users.Add(new_use);
            _db.SaveChanges();
